...
DIR_PATH = "/Users/prathakrastogi/Documents/SEM 1/Test/"
...
BWMAP_FILE      = DIR_PATH + "bw_map.txt"
########
...
def ngrams(data, labels, ntrain, mn=1, mx=1, nm=500, binary = False, donorm = False, stopwords = False, verbose = True, analyzer_char = False):
    ...
    if binary:
        vectorizer = CountVectorizer(max_df=mx,min_df=mn,binary=True)
    elif stopwords:
        vectorizer = TfidfVectorizer(max_df=mx,min_df=mn,stop_words='english',analyzer=analyzer_type,sublinear_tf=True)
    else:
...
def run(verbose = True):
    ...
    X_train4, y_train, X_test4 = ngrams(data, labels, ntrain, 1, 1, 2000, donorm = True, verbose = verbose, analyzer_char = True)    
    X_train5, y_train, X_test5 = ngrams(data, labels, ntrain, 2, 2, 4000, donorm = True, verbose = verbose, analyzer_char = True)    
    X_train6, y_train, X_test6 = ngrams(data, labels, ntrain, 3, 3, 100,  donorm = True, verbose = verbose, analyzer_char = True)    
