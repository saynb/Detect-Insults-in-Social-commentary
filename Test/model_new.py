...
def run(verbose = True):
    ...
    X_tn = sp.hstack([X_train1, X_train2, X_train3, X_train6, X_train7, X_train8])
    X_tt = sp.hstack([X_test1,  X_test2,  X_test3, X_test6, X_test7, X_test8])
...
def runClassifiers(X_train, y_train, X_test, y_test = None, verbose = True):
    ...
    models = [  linear_model.LogisticRegression(C=3), \
                ...
                ensemble.GradientBoostingClassifier(n_estimators=400, learning_rate=0.1, subsample = 0.5, min_samples_split=15, random_state = 100) \
              ]
